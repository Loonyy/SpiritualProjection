...
public class NPC {
    ...
    public static NPC getNPCByLocation(Location loc) {
        for(Map.Entry<Location, NPC> hm : npcsByLoc.entrySet()) {
            ...
            y = loc.getY();
            z = loc.getZ();
            if(newloc.equals(new Location(loc.getWorld(), x, y, z))) return hm.getValue();
        }
        return null;
    }
    ...
    public void despawn(Player p) {
        try {
            if(removePlayer == null) {
                ...
                removePlayer = getNMSClass("PacketPlayOutEntityDestroy").getConstructor(int[].class).newInstance(getArray(int.class, id));
            }
    ...
    public NPC(String name, Location loc, String skin, String vsName, String value, String signature) {
        try {
            ...
            TabPacket = Constructor.newInstance(ADD_PLAYER, getArray(EntityPlayerClass, EntityPlayer));
            SpawnPacket = getNMSClass("PacketPlayOutNamedEntitySpawn").getConstructor(EntityPlayer.getClass().getSuperclass()).newInstance(EntityPlayer);
            Constructor<?> cs = getNMSClass("PacketPlayOutEntity").getClasses()[0].getConstructor(int.class, byte.class, byte.class, boolean.class);
    ...
    public class ProfileLoader {
        ...
        private String getUUID(String name) {
            if(Bukkit.getOnlineMode()) {
                ...
                if(of != null) return of.getUniqueId().toString().replace("-", "");
            }
